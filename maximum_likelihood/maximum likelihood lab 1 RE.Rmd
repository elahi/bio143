...
sim_data$y <- a + b*sim_data$x + rnorm(n, mean = 0, sd = sigma)
...
Luckily, the Normal density function is already coded into `R` as the `dnorm` function, so we'll write a function for the negative log-likelihood using that. We can express the log-likelihood function very simply in R as:

```{}
-sum(dnorm(y, mean = ..., sd = ..., log = TRUE))
```

Where we'll provide `a + b*x` for the `mean`, and `sigma` for the `sd`. Since the standard deviation, $\sigma$, can't be negative, we'll return a very high value for any negative values of sigma:
...
sim_data %>%   
  ggplot(aes(x, y)) + # tell ggplot which aesthetics to use
  geom_point() + # add points
  geom_abline(intercept = a, slope = b, color = "red") + # show the known regression line
  geom_abline(intercept = fit$par[1], slope = fit$par[2], color = "blue") # our estimates
```
...
for (i in 1:nrow(par_grid)) {
  par_grid$loglik[i] <- lm_log_lik(
    c(a = par_grid$a[i], b = par_grid$b[i], sigma = fit$par[3]), 
    x = sim_data$x, 
    y = sim_data$y
  )
}
...
```{r lm_function}
lm_fit <- function(x, y, SE = "fisher", init = c("a" = 0, "b" = 0, "sigma" = 1), n_boot = 1000) {
...
Now, play around with simulating new data - can you recover the parameters that you simulated the data from? 

- Yes

What happens to the estimates and their SE when you decrease the sample size of the simulated data? 

- for the slope:
  - n = 50; 2.045; 0.08


Do the Fisher information and bootstrap methods agree on small datasets? On large ones?

They agree for large datasets; but for small datasets the SE's are larger for bootstrap
...
sim_data <- tibble(
  ...
  y = a + b*x + rnorm(n, mean = 0, sd = sigma) ## regression equation
)
...
lm_fit(sim_data$x, sim_data$y, SE = "bootstrap")
```
...
lm_fit(iris$Sepal.Length, iris$Petal.Length, SE = "fisher")

# Subset the data to setosa
...
full <- optim(par = c(0, 0, 1), fn = lm_log_lik, x = anscombe$x1, y = anscombe$y1)
full
full <- full$value
full
...
lm_log_lik_reduced <- function(pars, x, y) {
  a <- pars[1]; b <- 0; sigma <- pars[3]
  if (sigma > 0) {
    return(-sum(dnorm(y, mean = a + b * x, sd = sigma, log = TRUE)))
  } else {
    return(1000)
...
reduced <- optim(par = c(0, 0, 1), fn = lm_log_lik_reduced, 
...
## Comparison 
lm_full <- lm(y1 ~ x1, data = anscombe)
lm_reduced <- lm(y1 ~ 1, data = anscombe)
...
ml_fit <- optim(
  par = c(0, 0, 1), # initial values (a, b, sigma)
