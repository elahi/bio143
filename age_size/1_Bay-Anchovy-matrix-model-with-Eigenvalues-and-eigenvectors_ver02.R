...
# Let's create the Leslie matrix, four rows by four columns, initially populated
# by zeros
...
# let's plot our model into a pretty graphic to see stable age structure
years  <- 1:tmax # create an independent variable, years

#sf <- par(mfrow=c(2,2)) # let's make a 4-panel plot 
matplot(years,t(rbind(Ntot, nt)), type="l", lwd = 2, xlab="Year", ylab="Number of individuals in age class",main="Arithmetic scale") # note here we have to transpose our nt vector so the graph will work
legend(x=1, y=0.95*max(Ntot), legend=c("Ntot", "n1", "n2", "n3", "n4"), col=c(1,2,3,4,5), lty = c(1,2,3,4,5), lwd = 2)


matplot(years,t(rbind(Ntot, nt)), lwd = 2, type="l", xlab="Year", ylab="Number of individuals in age class", main="Logarithmic scale", log = 'y')
legend(x=1, y=5*max(Ntot), legend=c("Ntot", "n1", "n2", "n3", "n4"), col=c(1,2,3,4,5), lty = c(1,2,3,4,5), lwd = 2)


matplot(years,t(wt),type="l", lwd = 2,xlab="Year",ylab="Fraction of individuals in age class",main="Fractions in all stages")
legend(x=16, y=0.6, legend=c("n1", "n2", "n3", "n4"), col=c(1,2,3,4), lty = c(1,2,3,4), lwd = 2)

matplot(years,lamt,type="l", lwd = 2,xlab="Year",ylab="Lambda",main="Population growth rate")
...
# ASSIGNMENT                                                      #
#                                                                 # 
# - Change initial conditions and describe how long               #
#   the population takes to reach the stable age distribution     #
#                                                                 # 
###################################################################